...
class UrlBookmarkTest
  class SingleGateCircuitUrlBookmarkTest < ApplicationSystemTestCase
    setup do
  ...
  class MultiGateCircuitUrlBookmarkTest < ApplicationSystemTestCase
    test 'bookmark a C-1-H gate circuit' do
      visit new_circuit_path(json: '{"cols":[[1,1,1]]}')

      palette('control-gate').drag_to(first('circuit-dropzone'), html5: false)
      palette('h-gate').drag_to(third('circuit-dropzone'), html5: false)

      assert_equal '/new/{"cols":[["•",1,"H"]]}', decoded_current_path
    end
  end
end
