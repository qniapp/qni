...
QUnit.module('Simulator', () => {
  ...
  QUnit.module('.rnot', () => {
    ...
    QUnit.test("|1>.rnot(0) should be |1>.h(0).phase('π/2', 0).h(0)", assert => {
      assert.approximatelyEquates(new Simulator('1').rnot(0).state, new Simulator('1').h(0).phase('π/2', 0).h(0).state)
    })

    QUnit.test("|+>.rnot(0) should be |+>.h(0).phase('-π/2', 0).h(0)", assert => {
      assert.approximatelyEquates(new Simulator('+').rnot(0).state, new Simulator('+').h(0).phase('-π/2', 0).h(0).state)
    })
    ...
    QUnit.test("|i>.rnot(0) should be |i>.h(0).phase('π/2', 0).h(0)", assert => {
      assert.approximatelyEquates(new Simulator('i').rnot(0).state, new Simulator('i').h(0).phase('π/2', 0).h(0).state)
    })
  ...
  QUnit.module('.qft', () => {
    QUnit.test('|0>.qft(0) should be |+>', assert => {
      const simulator = new Simulator('0')
      assert.approximatelyEquates(simulator.qft(0).state, new StateVector('+'))
    })

    QUnit.test('|1>.qft(0) should be |->', assert => {
      const simulator = new Simulator('1')
      assert.approximatelyEquates(simulator.qft(0).state, new StateVector('-'))
    })

    QUnit.test('|+>.qft(0) should be |0>', assert => {
      const simulator = new Simulator('+')
      assert.approximatelyEquates(simulator.qft(0).state, new StateVector('0'))
    })

    QUnit.test('|->.qft(0) should be |1>', assert => {
      const simulator = new Simulator('-')
      assert.approximatelyEquates(simulator.qft(0).state, new StateVector('1'))
    })

    QUnit.test('|i>.qft(0) should be e^{iπ/4}|-i>', assert => {
      const π = Math.PI
      const i = Complex.I
      const e = new Complex(Math.E, 0)
    ...
    QUnit.test('|00>.qft(0) should be |0+>', assert => {
      const simulator = new Simulator('00')
      assert.approximatelyEquates(simulator.qft(0).state, new StateVector('0+'))
    })

    QUnit.test('|00>.qft(1) should be |+0>', assert => {
      const simulator = new Simulator('00')
      assert.approximatelyEquates(simulator.qft(1).state, new StateVector('+0'))
    })

    QUnit.test('|00>.qft(0, 1) should be |++>', assert => {
      const simulator = new Simulator('00')
      assert.approximatelyEquates(simulator.qft(0, 1).state, new StateVector('++'))
    })
  })
  ...
  QUnit.module('.qftDagger', () => {
    QUnit.test('|0>.qftDagger(0) should be |+>', assert => {
      const simulator = new Simulator('0')
      assert.approximatelyEquates(simulator.qftDagger(0).state, new StateVector('+'))
    })
    ...
    QUnit.test('|+>.qftDagger(0) should be |0>', assert => {
      const simulator = new Simulator('+')
      assert.approximatelyEquates(simulator.qftDagger(0).state, new StateVector('0'))
    })

    QUnit.test('|->.qftDagger(0) should be |1>', assert => {
      const simulator = new Simulator('-')
      assert.approximatelyEquates(simulator.qftDagger(0).state, new StateVector('1'))
    })
    ...
    QUnit.test('|i>.qftDagger(0) should be e^{iπ/4}|-i>', assert => {
      ...
      const simulator = new Simulator('i')
      assert.approximatelyEquates(
        simulator.qftDagger(0).state.matrix,
        new StateVector('(-i)').matrix.times(e.raisedTo(i.times(π).dividedBy(4)))
      )
    })
    ...
    QUnit.test('|00>.qftDagger(1) should be |+0>', assert => {
      const simulator = new Simulator('00')
      assert.approximatelyEquates(simulator.qftDagger(1).state, new StateVector('+0'))
    })

    QUnit.test('|00>.qftDagger(0, 1) should be |++>', assert => {
      const simulator = new Simulator('00')
      assert.approximatelyEquates(simulator.qftDagger(0, 1).state, new StateVector('++'))
    })
  })
