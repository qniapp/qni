...
export function testCircuitApi(uName, uClass) {
  ...
  it('disabled = true', function () {
    circuit[uName]({targets: [0], disabled: true})

    const operation = circuit.stepAt(0).dropzoneAt(0).operation
    assert.instanceOf(operation, uClass)
    assert.isTrue(operation.disabled)
  })
}
