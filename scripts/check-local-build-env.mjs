...
const toolVersions = read('.tool-versions')
  ...
  .filter(Boolean)
assert(
  toolVersions.includes('node 22.23.2'),
  `Expected .tool-versions to pin node 22.23.2, got: ${toolVersions.join(', ')}`
)
assert(
...
assert(
  read('.node-version') === '22.23.2',
  `Expected .node-version to be 22.23.2, got: ${read('.node-version')}`
)
...
assert(
  wwwPackage.engines?.node === '22.23.2',
  `Expected apps/www/package.json to pin node 22.23.2, got: ${wwwPackage.engines?.node}`
)
assert(
...
assert(ciWorkflow.includes('node-version: [22.23.2]'), 'Expected CI workflow to pin Node 22.23.2')
assert(
