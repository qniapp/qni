...
repo ルートの `package.json` は Yarn workspaces + Lerna を使って各 package/app をまとめて操作します。

主な役割:

- **Yarn workspaces**: `packages/*` と `apps/*` の依存解決を一元管理
- **Lerna**: `build`, `lint`, `typecheck`, `test`, `docs`, `size` を横断実行
...
- `yarn build:js`
- `yarn build:css`
- `yarn test`（system test を含む）
- `yarn test:ci`（Rails test）
- `yarn build:docs`（`apps/www/docs` の Jekyll build）
