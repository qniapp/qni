...
QUnit.module("Simulator", () => {
  ...
  QUnit.module(".x", () => {
    ...
    QUnit.test("|->.x(0) should be -|->", (assert) => {
      const simulator = new Simulator("-")
      assert.equates(
        simulator.x(0).state.matrix,
        new StateVector("-").matrix.times(-1),
      )
    })
    ...
    QUnit.test("|i>.x(0) should be i|-i>", (assert) => {
      ...
      assert.equates(
        simulator.x(0).state.matrix,
        new StateVector("-i").matrix.times(new Complex(0, 1)),
      )
    })
  ...
  QUnit.module(".phase", () => {
    ...
    QUnit.test("|1>.phase('pi', 0) should be -|1>", (assert) => {
      const simulator = new Simulator("1")
      assert.approximatelyEquates(
        simulator.phase("pi", 0).state.matrix,
        new StateVector("1").matrix.times(-1),
      )
    })
