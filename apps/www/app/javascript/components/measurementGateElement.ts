...
export class MeasurementGateElement extends ConfigurableMixin(
  DragAndDroppableMixin(IconableMixin(HelpableMixin(LabelableMixin(SizeableMixin(JsonableMixin(HTMLElement))))))
) {
  ...
  update(): void {
    render(
      html`${this.sizeableStyle} ${this.iconStyle} ${this.dragAndDroppableStyle} ${this.labelStyle}
  ...
  get iconSvg(): TemplateResult {
    return html`
      ...
      >
        ...
        <circle cx="24.5" cy="33" r="1.5" fill="currentColor" stroke="currentColor" />
      </svg>
    `
  }
}
